...
const process = {
  findAllScrapsByClubNum: async (req, res) => {
    ...
    if (response.success) {
      logger.info(`GET /api/my-page/id/personal/clubNum 200: ${response.msg}`);
    ...
    logger.error(`GET /api/my-page/id/personal/clubNum 404: ${response.msg}`);
  ...
  findOneScrap: async (req, res) => {
    ...
    if (response.success) {
      logger.info(
        `GET /api/my-page/id/personal/scrap/clubNum/scrpaNum 200: ${response.msg}`
      );
      return res.status(200).json(response);
    }
    if (response.isError) {
      logger.error(
        `GET /api/my-page/id/personal/scrap/clubNum/scrpaNum 500: \n${response.errMsg}`
      );
      return res.status(500).json(response.clientMsg);
    }
    logger.error(
      `GET /api/my-page/id/personal/scrap/clubNum/scrpaNum 404: ${response.msg}`
    );
    return res.status(404).json(response);
  },
